...
function removePageDataAds(items) {
  ...
  for (let i = items.length - 1; i >= 0; i--) {
    ...
    for (const category of item.categories || []) {
      ...
      category.pageDatas = category.pageDatas.filter((pageData) => {
        const isSuperTopic = pageData?.gid === "110057182642782"
          || pageData?.fid === "110057182642782"
          || pageData?.title === "超话社区"
          || pageData?.navigation_title === "超话社区";
        if (isSuperTopic) {
          console.log('移除首页顶部超话社区按钮');
        }
        return !isSuperTopic;
      });
    }
